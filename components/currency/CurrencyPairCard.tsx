...
/**
 * One FX pair: a compact price line plus a Technical Analysis gauge that
 * summarizes the key oscillators / moving averages into a Buy–Sell expectation.
 * Both widgets lazy-mount when the card scrolls near the viewport.
 */
export default function CurrencyPairCard({ pair }: { pair: CurrencyPair }) {
    ...
    return (
        <div className="flex flex-col rounded-xl border border-gray-800 bg-gray-900/40 p-4">
            <div className="mb-3 flex items-baseline justify-between gap-2">
                <h3 className="text-base font-semibold text-gray-100">{pair.code}</h3>
                ...
                {visible ? (
                    <div className="flex flex-col gap-3">
                        <TradingViewWidget
                            ...
                            config={MINI_SYMBOL_WIDGET_CONFIG(pair.symbol)}
                            height={MINI_HEIGHT}
                        />
                        <TradingViewWidget
                            scriptUrl={`${SCRIPT}technical-analysis.js`}
                            config={TECHNICAL_ANALYSIS_WIDGET_CONFIG(pair.symbol)}
                            height={GAUGE_HEIGHT}
                        />
                    </div>
                ) : (
