...
import { ETF_CATEGORIES, categoryEtfCount, getEtfCategory } from '@/lib/etfs';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const cat = getEtfCategory(category);
    if (!cat) return { title: 'ETF Hub' };
    const count = categoryEtfCount(cat);
    return {
        title: `${cat.label} ETFs`,
        description: `${count} ${cat.label} ETFs with live multi-period returns — ${cat.blurb}`,
    };
}
...
export default async function EtfCategoryPage({ params }: Props) {
    const { category } = await params;
    const cat = getEtfCategory(category);
    if (!cat) notFound();
    const count = categoryEtfCount(cat);
    ...
    const breadcrumbLd = {
        ...
        itemListElement: [
            ...
            { '@type': 'ListItem', position: 3, name: cat.label, item: `${SITE_URL}/etfs/${cat.id}` },
        ],
    };
    const itemListLd = {
        '@context': 'https://schema.org',
        ...
        name: `${cat.label} ETFs`,
        numberOfItems: count,
        itemListElement: cat.groups.flatMap((g) => g.etfs).map((etf, i) => ({
            '@type': 'ListItem',
            position: i + 1,
            name: `${etf.symbol} — ${etf.name}`,
            url: `${SITE_URL}/stocks/${etf.symbol}`,
        })),
    };
        ...
        <div className="flex min-h-screen w-full flex-col gap-6 p-4 md:p-8">
            ...
            <header className="flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <p className="max-w-3xl text-sm text-gray-400">
                        {cat.intro} <span className="text-gray-500">({count} funds)</span>
                    </p>
                </div>
                {cat.caution && (
            ...
            {cat.groups.map((group) => (
                <section key={group.id} className="flex flex-col gap-3">
                    ...
                    <Suspense fallback={<TableSkeleton rows={group.etfs.length} />}>
                        <SymbolGroupTable
                            rows={group.etfs.map((etf) => ({ symbol: etf.symbol, name: etf.name }))}
                            nameLabel="Fund"
                        />
                    </Suspense>
                </section>
            ...
            <p className="text-[11px] text-gray-600">
                Returns approximated from adjusted end-of-day closes. Directory is curated for
                liquidity and demand, not exhaustive; fund lineups change — verify before trading.
            </p>

            <RelatedLinks
                items={[
                    { href: '/etfs', label: 'ETF Hub', desc: 'Back to all categories' },
                    { href: '/compare', label: 'Compare', desc: 'Chart two of these funds head-to-head' },
